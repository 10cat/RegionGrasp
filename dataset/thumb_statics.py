    ...
    def fill_interior(self, hand_mesh, obj_mesh, face_ids_dict):
        ...
        while(verts_in_search):
            ...
            for idx, vid in enumerate(verts_in_search):
                ...
                # import pdb; pdb.set_trace()
                # FIXME: 想要用快一点的批量筛选verts方法，但是没有办法完整fill
                # v_fids = v_fids[v_fids > 0].tolist() # unpadded
                # v_fids = list(set(v_fids) - set(face_ids))
                # vids = obj_mesh.faces[v_fids].reshape(-1).tolist()
                # vids = list(set(vids))
                # vids = list(set(vids) - set(vert_ids))
                # if vids:
                #     inner_vids = inner_verts_detect(hand_mesh, obj_mesh, vids)
                #     face_ids += v_fids
                #     face_ids_dict[self.hand_comps_name[comp]] += v_fids
                #     vert_ids += inner_vids
                #     new_vert_ids += inner_vids
                #     new_vert_comps += [comp] * len(inner_vids)
