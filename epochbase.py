...
class Epoch(nn.Module):
    ...
    def init_trainutils(self):
        ...
        self.ConditionNetLoss = ConditionNetLoss()
        self.cGraspVAELoss = cGraspvaeLoss()

        self.ConditionNetMetrics = ConditionNetMetrics()
    ...
    def loss_compute(self, outputs, data):
        ...
        dict_losses['total_loss'] = torch.sum(dict_losses.values())
    ...
    def metrics_log(self, epoch):
        ...
        metric_message = self.log_monitor.metric_message(self.Metrics, epoch)
        print(loss_message, metric_message)
        AllMeters = self.Losses.update(self.Metrics)
        # turn averagemeters to
        wandb.log(AllMeters.avg)

    def save_checkpoints(self, epoch, best_val):
        if epoch > 1 and epoch % cfg.check_interval == 0:
            checkpoint_path = (self.model_root, f'checkpoint_{epoch}.pth')
            torch.save(self.ConditionNet.state_dict(), checkpoint_path)

    
    def epoch(self, epoch, best_val=None):
...
class ValEpoch(Epoch):
    ...
    def save_checkpoints(self, epoch, best_val):
        if cfg.fit_Condition and not cfg.fit_cGrasp:
            metric_val = self.Metrics['metric_cond']
            if best_val is not None and metric_val > best_val:
                checkpoint_path = (self.model_root, f'bestmodel_{epoch}.pth')
                torch.save(self.ConditionNet.state_dict(), checkpoint_path)
    ...
    def model_forward(self, data):
        ...
        # 2) forward the cGraspvae model to obtain hand parameters as the key prediction
        condition_vec, SD_maps, hand_params, sample_stats = None, None, None, None
        feats, SD_maps = self.ConditionNet.inference(obj_vs, rhand_vs, region) # cSDF_maps = [pred_map_obj, pred_map_obj_masked]
        condition_vec = feats[0]
        if self.cGraspVAE:
            hand_params, sample_stats = self.cGraspVAE([obj_vs, rhand_vs], condition_vec)
